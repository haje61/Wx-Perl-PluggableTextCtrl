package Wx::Perl::PluggableTextCtrl;
...
require Wx::Perl::PluggableTextCtrl::KeyEchoes;
require Wx::Perl::PluggableTextCtrl::UndoRedo;
require Wx::Perl::PluggableTextCtrl::Highlighter;
...
sub LoadPlugin {
   ...
   while (@_) {
      ...
      $name = "Wx::Perl::PluggableTextCtrl::$name";
      $plug = $name->new($self);
#      eval "require Wx::Perl::PluggableTextCtrl::$name; \$plug = Wx::Perl::PluggableTextCtrl::$name" . "->new(\$self);";
      if (defined($plug)) {
...
sub SaveFile {
   my $self = shift;
   unless ($self->Broadcast('replace', @_)) {
      $self->SUPER::Replace(@_);
   }
}
